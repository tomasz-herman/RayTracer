...
namespace rt {
    class Sampler2 {
    public:
        Sampler2(const std::function<std::valarray<Vec2>(int)> &generator, int count, int sets = 1, const std::function<Vec2(Vec2)> &mapping = std::identity());
    ...
    private:
        ...
        std::valarray<std::valarray<Vec2>> samples;
